...
def setup_server():
    ...
    # Create database
    sudo('echo "CREATE DATABASE hemedicinal; '
               'CREATE USER hemedicinal WITH password \'%s\'; '
               'GRANT ALL PRIVILEGES ON DATABASE hemedicinal to hemedicinal;" '
    ...
    put(local_path=StringIO(local_settings), remote_path=env.code_dir + '/hemedicinal/settings/local.py', use_sudo=True)

    # Create necessary dirs, with correct permissions
    mkdir_wwwdata('/var/log/hemedicinal/')
    with cd(env.code_dir + '/hemedicinal'), prefix('. ../venv/bin/activate'):
    ...
    sudo('chown -R www-data:www-data /var/log/hemedicinal/')

    # Copy nginx and gunicorn confs
    with cd(env.code_dir):
        sudo('cp deploy/nginx.conf /etc/nginx/sites-enabled/hemedicinal')
        sudo('cp deploy/gunicorn.conf /etc/init/gunicorn-hemedicinal.conf')
...
def collectstatic():
    with cd(env.code_dir + '/hemedicinal'):
